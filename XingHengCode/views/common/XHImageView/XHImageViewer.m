...
- (void)actionSheet:(UIActionSheet*)actionSheet didDismissWithButtonIndex:  (NSInteger)buttonIndex{
    if(buttonIndex ==0) {
        UIImageView *img = [self currentView];
        UIImageWriteToSavedPhotosAlbum(img.image, self, @selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:), nil);
    }
}
...
- (void)dismissWithAnimate {
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
                         ...
                         XHViewState *state = nil;
                         for(UIImageView *imageView in _imgViews) {
                             state = [XHViewState viewStateForView:imageView];
                             if(state.superview) {
                                 break;
                             }
                         }
                         CGRect rect = [window convertRect:state.frame fromView:state.superview];
