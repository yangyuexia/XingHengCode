...
static const CGFloat kHeightOfTopScrollView = 39.0f;
static const CGFloat kWidthOfButtonMargin = 1.0f;// 2.0f
...
- (void)showMessageLabelWithTag:(NSInteger)tag withNumber:(NSString *)msgNumber{
    
    UILabel *messageLabel = [self viewWithTag:(tag + 500)];
    messageLabel.text = msgNumber;
    messageLabel.hidden = NO;
}

- (void)hideMessageLabelWithTag:(NSInteger)tag{
...
- (void)createNameButtons
{
    ...
    _shadowImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 4, 59, 35)];
    [_shadowImageView setImage:_shadowImage];
    [_topScrollView addSubview:_shadowImageView];
    CGFloat buttonWidth = 0.0f;
    
    buttonWidth = APP_W / [_viewArray count] - kWidthOfButtonMargin * 2;
    if(!self.isIndentAdjust){
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        UILabel  *messageLabel = [[UILabel alloc]initWithFrame:CGRectMake(xOffset + buttonWidth - 40, 9.5f, 12, 12)];
        messageLabel.font = fontSystem(kFontS2);
        messageLabel.textAlignment = NSTextAlignmentCenter;
        messageLabel.textColor = RGBHex(qwColor4);
        messageLabel.layer.masksToBounds = YES;
        messageLabel.layer.cornerRadius = 6.0f;
        messageLabel.backgroundColor = RGBHex(qwColor3);
        messageLabel.tag = i + 500;
        messageLabel.hidden = YES;
        [_topScrollView addSubview:messageLabel];
    
        
        CGSize textSize = [vc.title sizeWithFont:self.tabItemNormalFont
        ...
        if (i == 0) {
            if([_viewArray count] == 1)
                _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 4,buttonWidth, _shadowImage.size.height);
            else
                _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 4,buttonWidth, _shadowImage.size.height);
//            button.selected = YES;
        ...
        button.titleLabel.font = self.tabItemNormalFont;
    ...
    if (_viewArray.count > 4) {
        ...
        _topScrollView.contentSize = CGSizeMake(APP_W, kHeightOfTopScrollView);//CGSizeMake(320, kHeightOfTopScrollView);
    }
...
//    }
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        if (i == 0) {
            if([_viewArray count] == 1)
                _shadowImageView.frame = CGRectMake(kWidthOfAdjustHealthIndicator, 4,textSize.width, _shadowImage.size.height);
            else
                _shadowImageView.frame = CGRectMake(kWidthOfAdjustHealthIndicator, 4,textSize.width, _shadowImage.size.height);
            //            button.selected = YES;
...
//        [self adjustScrollViewContentX:sender];
        ...
        if (sender.tag != _userSelectedChannelID) {
            ...
            lastButton.titleLabel.font = self.tabItemNormalFont;
            //赋值按钮ID
        ...
        if (!sender.selected) {
            sender.selected = YES;
            sender.titleLabel.font = self.tabItemSelectFont;
...
//                }
//            }];
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 4, sender.frame.size.width, _shadowImage.size.height)];
            if (!_isRootScroll) {
