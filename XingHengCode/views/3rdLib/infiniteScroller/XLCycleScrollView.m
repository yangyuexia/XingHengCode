...
- (void)setupUIControl
{
    ...
    if (!self.pageHidden) {
        ...
        rect.origin.x = 10;
        rect.size.width = APP_W-20;
        rect.size.height = 30;
        _pageControl = [[UIPageControl alloc] initWithFrame:rect];
        _pageControl.pageIndicatorTintColor = RGBAHex(qwColor5, 0.5);
        _pageControl.currentPageIndicatorTintColor = RGBHex(qwColor20);
...
- (void)reloadData
{
    ...
    if ([_datasource respondsToSelector:@selector(showBottomScrollLabel)]) {
        if ([_datasource showBottomScrollLabel]) {
            ...
            [_pageControl setFrame:CGRectMake(0, _scrollView.frame.origin.y + _scrollView.frame.size.height - 55, APP_W, 25)];
        }
    }
    _curPage = 0;
...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView
{
    ...
    if(x >= (2*self.frame.size.width)) {
        NSInteger changePage = [self validPageValue:_curPage+1];
        if(changePage != _curPage && !pageChange) {
            _curPage = changePage;
            pageChange = YES;
        }
        [self loadData];
