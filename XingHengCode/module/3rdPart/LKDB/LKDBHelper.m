...
- (instancetype)init
{
    return [self initWithDBName:@"QWYF"];
}
...
-(void)setDBPath:(NSString *)filePath
{
    ...
    if(lastComponent.length > 0)
    {
        ...
        if ( isCreated == NO || isDir == NO )
        {
            ...
            if(success == NO)
            {
                DDLogVerbose(@"create dir error: %@",error.debugDescription);
            }
        }
    }
...
-(BOOL)_createTableWithModelClass:(Class)modelClass tableName:(NSString*)tableName
{
    ...
    for (NSInteger i=0; i<infos.count; i++)
    {
        ...
        {
            //6>>6去掉将 double 映射为 sqlite text 的限制
...
{
    return [self _createTableWithModelClass:modelClass tableName:tableName];
}
